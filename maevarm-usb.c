...
{
	char a,run=0;
	if((a = i/10000))				// if the MSD is non-zero
	{
		usb_tx_char('0' + a);
		run = 1;
	}
	if((a = (i%10000)/1000) || run)	// if we're running or digit is non-zero
	{
		usb_tx_char('0' + a);
		run = 1;
	}
	if((a = (i%1000)/100) || run)	// if we're running or digit is non-zero
	{
		usb_tx_char('0' + a);
		run = 1;
	}
	if((a = (i%100)/10) || run)	// if we're running or digit is non-zero
	{
		usb_tx_char('0' + a);
		run = 1;
	}
	usb_tx_char('0' + (i%10));	// always send the LSD
	
//	usb_tx_char('0' + i/10000);
//	usb_tx_char('0' + (i%10000)/1000);
//	usb_tx_char('0' + (i%1000)/100);
//	usb_tx_char('0' + (i%100)/10);
//	usb_tx_char('0' + (i%10));
}